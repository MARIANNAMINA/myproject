<?php
/*
 * Contains the functionality of the button Return from break of the file clock_in_employee.php.
 */
session_start();
...
if (isset($_POST['returnFromBreak'])) {
	...
	// used to check if employee attempts to click the button Return from break without pressing first the button Clock in for the current date
    $flag = true;
    ...
    if (!$sql || !$sql2) {
        ...
        while ($row = mysqli_fetch_array($sql)) {
            $flag = false;
			// check if the result of the above query, specifically ClockOut column has not its default value, employee attempts to click Return from break button without pressing first the button Clock in
            if (strcmp($row['ClockOut'], "00:00:00") != 0) {
               print_error_retbreak_in();
			// check if the result of the above query, specifically if value of Return from break column is greater than the value of Break column, employee attempts to click Return from break button as he/she is clocked in   
            } elseif ($row['Break'] < $row['ReturnBreak']) {
                print_error_retBreak();
			// check if the result of the above query, specifically if value of Break column has its default value, employee attempts to click Return from break button without pressing the Break button   		
            } elseif (strcmp($row['Break'], "00:00:00") == 0) {
              print_error_ret_break();
            } else {
				$query="";
				if(!$flag2){
					// update the time that employee clicked the button Return from break with the current one
		...
		// check if $flag is true, employee attempts to click the button Return from break without pressing first the button Clock in for the current date
        if ($flag) {
            print_error_retbreak_in();
        }
    }
...
/**
 * Prints an error message related to that an employee tries to click the button Return from Break without press the button *Clock in first 
 */
...
/**
 * Prints an error message related to that an employee tries to click the button Return from Break up to one time without pressing first the button Break
 */
...
/**
 * Prints an error message related to that an employee tries to click the button Return from Break without pressing first the button Break
 */
...
/**
 * Calculates the last break length of the employee
 * @param $row_break_len Contains the time that employee clicks the button Return from Break and the time that employee pressed the button Break
 * @return int Returns how much time employee was in break
 */
...
/**
 * Insert in the database how much time employee was on break for the last time that he/she did a break
 * @param $conn The connection with the database
 * @param $break The length of the break
 * @param $flag2 Used to check if employee is clocked in from a previous date or not
